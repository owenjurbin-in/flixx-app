...
async function displayTrendingMovies() {
    const { results }= await fetchData('trending/movie/week')
    ...
    results.forEach(movie => {
        ...
        `<a href="movie-details.html?id=${movie.id}">
             ${ 
                 movie.poster_path 
                 ? `<img 
                 src="https://image.tmdb.org/t/p/w500${movie.poster_path}" class="card-img-top" alt="${movie.title}"/>` : `<img src="../images/no-image.jpg" class="card-img-top" alt="${show.title}"/>`}
           </a>
           <div class="card-body">
...
// displays 20 most popular tv shows 

async function displayTrendingShows() {
    const { results }= await fetchData('trending/tv/week')
    ...
    results.forEach(show => {
        ...
        `<a href="tv-details.html?id=${show.id}">
             ${ 
                 show.poster_path 
                 ? `<img 
                 src="https://image.tmdb.org/t/p/w500${show.poster_path}" class="card-img-top" alt="${show.name}"/>` : `<img src="../images/no-image.jpg" class="card-img-top" alt="${show.name}"/>`}
           </a>
           <div class="card-body">
...
async function displayMovieDetails() {
    ...
    `   <div class="details-top">  
        <div>
          ${movie.poster_path ? `<img src="https://image.tmdb.org/t/p/w500${movie.poster_path}" class="card-img-top" alt="${movie.title}"/>` : `<img src="../images/no-image.jpg" class="class-img-top" alt="${movie.title}"/>`} 
          </div>
          <div>
            <h2>${movie.title}</h2>
            <p>
...
async function displayShowDetails() {
    ...
    `<div class="details-top">
          <div>
          ${show.poster_path ? `<img src="https://image.tmdb.org/t/p/w500${show.poster_path}" class="card-img-top" alt="${show.name}"/>` : `<img src="../images/no-image.jpg" class="class-img-top" alt="${show.name}"/>`} 
          </div>
          <div>
            <h2>${show.name}</h2>
            <p>
        ...
        <div class="details-bottom">
          ...
          <ul>
            ...
            <li>
              <span class="text-secondary">Last Episode To Air:</span> ${show.last_episode_to_air.name}, ${show.last_air_date}
                          </li>
...
async function displayBackgroundImage(type, backdrop_path) {
...
async function fetchData(endpoint) {
    const API_KEY = '826d57da6bf79f044d298a73055f8950'
    ...
    results.forEach((result) => {
        ...
        `<a href="${global.search.type}-details.html?id=${result.id}">
             ${ 
                 result.poster_path 
                 ? `<img 
                 src="https://image.tmdb.org/t/p/w500/${result.poster_path}" class="card-img-top" alt="${global.search.type === 'movie' ? result.title : result.name}"/>` : `<img src="../images/no-image.jpg" class="card-img-top" alt="${global.search.type === "movie" ? result.title : result.name}"/>`
             }
            </a>
            <div class="card-body">
             <h5 class="card-title">${global.search.type === 'movie' ? result.title : result.name}</h5>
             <p class="card-text">
               <small class="text-muted">Release: ${global.search.type === 'movie' ? result.release_date : result.name}</small>
             </p>
           </div>
                 `
